...
$p = Start-Process -Wait -PassThru -FilePath msiexec -ArgumentList "/i C:\Windows\Temp\cloudbase-init.msi /qn /norestart /l*v C:\Windows\Temp\cloudbase-init.log LOGGINGSERIALPORTNAME=$serialPortName USERNAME=packer NETWORKADAPTERNAME=""Intel(R) PRO/1000 MT Network Connection"""
if ($p.ExitCode -ne 0) {
